MMR.initializeDetailPage = async () => {
    ...
    const generateCellId = (roomId, targetDatetime) => {
        ...
        const timeStrSimple = `${MMR.padToTwo(
            targetDatetime.getHours()
        )}${MMR.padToTwo(targetDatetime.getMinutes())}`;
    ...
    const createGrid = (
        ...
        for (let i = 0; i < areas.length; i++) {
            const roomInCurrentArea = reservationRooms.filter(
                (room) => room.areaId === areas[i].areaId
            );
            ...
            for (let j = 0; j < roomCount; j++) {
                ...
                for (
                    let k = fromDateValue.getTime();
                    k <= toDateValue.getTime();
                    k += ONE_DAY_IN_MILLIS
                ) {
                ...
                for (
                    ...
                    for (let t = fromTimeValue; t <= toTimeValue; t++) {
                        for (let x = 0; x <= 45; x += 15) {
                            ...
                            const dayStrSimple = MMR.formatDateSimple(
                                targetDate
                            );
                            const timeStr = `${MMR.padToTwo(t)}:${MMR.padToTwo(
                                x
                            )}`;
                            const timeStrSimple = `${MMR.padToTwo(
                                t
    ...
    const refreshReservations = async () => {
        ...
        const reservationToDatetime = new Date(
            new Date(reservationTo).getTime() + 60 * 60 * 1000
        );
        ...
        reservations.forEach((reservation) => {
            ...
            for (
                let k = reservedFrom.getTime();
                k < reservedTo.getTime();
                ...
                if (cell) {
                    ...
                    if (k === reservedFrom.getTime()) {
                        ...
                        k ===
                        reservedTo.getTime() - FIFTEEN_MINUTES_IN_MILLIS
                    ) {
    ...
    const mouseEvents = (detailDiv) => {
        ...
        detailDiv.addEventListener("mousedown", async (e) => {
            ...
            if (cell) {
                if (isReserved(cell)) {
                    ...
                    const success = await RESERVATION.editReservation(
                        reservation
                    );
                    if (success) {
                        refreshReservations();
        ...
        detailDiv.addEventListener("mousemove", (e) => {
            ...
            if (dragging) {
                ...
                if (
                    cell.getAttribute("rowKey") !==
                    startingPoint.getAttribute("rowKey")
                ) {
                ...
                if (selectedCells[cell.id]) {
                    ...
                    if (direction !== 0) {
                        for (const key in selectedCells) {
                            ...
                            const selectedNo =
                                selectedCell.getAttribute("no") - 0;
                ...
                if (Math.abs(previousNo - currentNo) === 1) {
                    ...
                    const previousDatetime = new Date(
                        previousPoint.getAttribute("datetime")
                    );
                    ...
                    if (direction > 0) {
                        for (
                            ...
                            const middleCellId = generateCellId(
                                roomId,
                                new Date(k)
                            );
        ...
        window.addEventListener("mouseup", async (e) => {
            if (dragging) {
                ...
                const toDatetimeValue = new Date(
                    cellTo.getAttribute("datetime")
                );
                // To should be set as start of the next cell, so plus 15 minutes
                const to = MMR.formatDateFull(
                    new Date(
                        toDatetimeValue.getTime() + FIFTEEN_MINUTES_IN_MILLIS
                    )
                );
                const success = await RESERVATION.editReservation(
                    { from, to, roomId, roomName },
                    () => {}
                );
                if (success) {
                    refreshReservations();
        ...
        methods: {
            ...
            reload: async function () {
                ...
                this.areaSelected.forEach((area) => {
                    const newAreaDetail = areas.find(
                        (areaOption) => areaOption.areaId === area.areaId
                    );
                    if (newAreaDetail) {
                ...
                this.roomSelected.forEach((room) => {
                    const newRoomDetail = rooms.find(
                        (roomOption) => roomOption.roomId === room.roomId
                    );
                    if (newRoomDetail) {
            ...
            checkRequired: function (value, ref) {
                if (value == null || value === "" || value === undefined) {
                    MMR.showNotification(
                        MMR.currentLangResource.errorRequired,
                        ""
                    );
                    ref.focus();
                    return false;
            ...
            search: async function () {
                ...
                if (!this.areaSelected || this.areaSelected.length == 0) {
                    MMR.showNotification(
                        MMR.currentLangResource.errorAreaRequired,
                        ""
                    );
                    return;
                }
                ...
                if (isNaN(fromDateValue.getTime())) {
                    MMR.showNotification(
                        MMR.currentLangResource.errorInvalidDate,
                        ""
                    );
                ...
                if (term < 0) {
                    MMR.showNotification(
                        MMR.currentLangResource.errorInvalidTermDate,
                        ""
                    );
                    this.$refs.fromDate.focus();
                    return;
                ...
                if (term > 30 * 24 * 60 * 60 * 1000) {
                    ...
                    MMR.showNotification(
                        MMR.currentLangResource.errorTermTooLongForSearch,
                        ""
                    );
                    this.$refs.fromDate.focus();
                    return;
                ...
                if (fromTimeValue >= toTimeValue) {
                    ...
                    MMR.showNotification(
                        MMR.currentLangResource.errorInvalidTermTime,
                        ""
                    );
